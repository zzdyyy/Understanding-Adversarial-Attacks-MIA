...
def load_data(batch_size, mixup, vFlip, rotation):
    ...
    if mixup:
        ...
        X_train = np.load(data_dir + 'ub3_056_train_x.npy')  # N x 224 x 224 x 3
        y_train = np.load(data_dir + 'ub3_056_train_y.npy')  # N x c
        X_val = np.load(data_dir + 'ub3_056_val_x.npy')
        y_val = np.load(data_dir + 'ub3_056_val_y.npy')
...
def generateCallbacks(inceptionModel, nameAppend, LR, modelChecking, modelCheckpointPeriod, earlyStopping, earlyStopPatience):
    ...
    if inceptionModel:
        ...
        runId = "ResNet50"
    runId += "_" + nameAppend + time.strftime('%m.%d_%H.%M.%S')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rotation", type=int, default=20,  help="Degree of rotation during data aug")
