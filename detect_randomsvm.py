...
def get_random_svms(dataset, attack, *cfg):
    ...
    svm_file = 'data/rsvms_%s_%s.model' % (dataset, attack)
...
def detect(args):
    ...
    if do_pca:
        pca = PCA(20, True)
        X_pca = pca.fit_transform(cX_train)
        print('PCA explanation of variances:', (pca.explained_variance_ratio_))
        plt.scatter(X_pca[cy_train<1, 0], X_pca[cy_train<1, 1])
        plt.scatter(X_pca[cy_train>0, 0], X_pca[cy_train>0, 1])
        plt.show()

    rsvms = get_random_svms(args.dataset, args.attack, important_ratio, svm_dim, svm_num, svm_imp_ratio, cX_train, cy_train)
    ...
    detector = RandomForestClassifier(30)  # SVC(gamma=2.8, probability=True)
